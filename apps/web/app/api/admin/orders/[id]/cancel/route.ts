...
// Cancel order (PENDING / CONFIRMED / PACKED) and restock.
export const POST = route(async (req, ctx) => {
  ...
  try {
    const order = await orders.adminCancel(id, body.reason);
    after(() => notifications.sendOrderStatusUpdate(order.id, 'CANCELLED').catch((e) => console.error(e)));
