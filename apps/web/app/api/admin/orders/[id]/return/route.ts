...
// Mark a SHIPPED/DELIVERED order as returned + restock.
export const POST = route(async (req, ctx) => {
  await requirePermission(req, 'manage_orders');
  const { id } = await parseParams(ctx, IdParam);
  const body = await parseBody(req, ReturnBody);
  const orders = new OrderService(prisma);
  try {
    return json(serializeMoney(await orders.adminMarkReturned(id, body.reason)));
  } catch (err) {
